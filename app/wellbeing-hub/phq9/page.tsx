...
export default function PHQ9Assessment() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/questions/phq9`)
        const data = await response.json()
  ...
  const handleNext = async () => {
    if (currentQuestion < (assessmentData?.questions.length || 0) - 1) {
      ...
      try {
        ...
        // Format answers with sequential numbers from 1 to 9
        const formattedAnswers = assessmentData?.questions.reduce((acc, question, index) => {
          acc[index + 1] = answers[index]  // Use index + 1 to get numbers 1-9
          return acc
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/assessments/phq9`, {
          method: 'POST',
          headers: {
            ...
            >
              ...
              <CardContent>
                <p className="text-gray-600">
                  This version contains the standard PHQ-9 questions used for depression screening.
                </p>
              </CardContent>
              <CardFooter>
                ...
                <Button 
                  ...
                  onClick={async () => {
                    if (selectedTherapist) {
                      try {
                        setIsLoading(true)
                        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/questions/phq9?therapist_id=${selectedTherapist}`)
                        const data = await response.json()
                        ...
                        if (data.success) {
                          if (!data.data.questions || data.data.questions.length === 0) {
                            throw new Error('Therapist has not set up any questions for the PHQ-9 assessment. Please contact your therapist to set up the assessment questions.')
                          }
